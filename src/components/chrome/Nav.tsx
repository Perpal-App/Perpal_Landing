 ...
 *
 * On load it crossfades in as the opening card clears the top of the viewport, and that
 * is the whole of it — no shape animation, on instruction. Two were tried and removed;
 * the note on `nav-reveal` in `globals.css` records what each cost, so neither gets
 * reinvented.
 *
 * `backwards`, not `both`: the `from` state covers the delay, so the capsule is unpainted
